...
------------------------ with VOID , NO return --------------------------
DROP FUNCTION late_fee; --DOES NOT WORK, HAS NO RETURN but updates system
...
SELECT late_fee(17503, 341, 1.00);

SELECT *
FROM payment 
WHERE payment_id = '17503';

---DOES NOT WORK - RETURNS MORE THAN 1 ROW?rounding now
DROP FUNCTION late_fee;
CREATE OR REPLACE FUNCTION late_fee (
	_payment_id INTEGER,      -- <-PARAMETERS put line BEFORE, name anything I want
	_customer_id INTEGER,
...
--class exercise -- works-look for specific id
...
SELECT actor_last_name(7, 'Jones');

SELECT *
FROM actor ;

----------- Will's example ------ change last name --------------WORKS
...
--updates RETURN date ON rental table-WORKS
...
--add actor into actor table-WORKS
...
-------DELETE entry----got lucky oes not work
